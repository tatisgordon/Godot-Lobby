...
var currentPlayerNumberPositionCount = 0
# This will contain player info for every player,
# with the keys being each player's unique IDs.
var playersReadyCount = 0
var players = {}  #lust of connected players
var peerList: Array = []  #list of peers id
...
func increasePlayerCount():
	if multiplayer.is_server():
		currentPlayerNumberPositionCount += 1
...
func orderPlayers():
	peerList.sort()
	for x in range(peerList.size()):
		var index = peerList[x]
		players[index].playerNumber = x
...
func getPlayerNetworData():
	...
	return {"name": _name, "ready": _readyStatus, "playerNumber": currentPlayerNumberPositionCount}
